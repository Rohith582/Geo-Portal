...
function fetchDistricts(selectedState) {
    var wfsUrl = "http://localhost:8080/geoserver/India/wfs?service=WFS&version=1.0.0&request=GetFeature&typeName=India:Health2&" +
        "outputFormat=application/json&propertyName=lgd_distri&CQL_FILTER=state='" + selectedState + "'";

    fetch(wfsUrl)
        .then(response => response.json())
        .then(data => {
            var districts = data.features.map(feature => feature.properties.lgd_distri);
...
function zoomToDistrict(districtName) {
    ...
    var url = "http://localhost:8080/geoserver/India/wfs?service=WFS&version=1.0.0&request=GetFeature&typeName=India:Health2&" +
        "outputFormat=application/json&cql_filter=lgd_distri='" + districtName + "'";
...
function filterByDistrict() {
    if (selectedDistrict) {
        var query = "lgd_distri='" + selectedDistrict + "'";
